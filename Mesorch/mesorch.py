...
This file implements a hybrid CNN-Transformer model for image manipulation localization.
"""
...
from IMDLBenCo.IMDLBenCo.registry import MODELS

# Frequency feature extractors (assumed to be defined elsewhere)
...
class ConvNeXt(timm.models.convnext.ConvNeXt):
    """
    Modified ConvNeXt backbone with extended input channels (e.g., for DCT features).
    """
    ...
    def __init__(self, conv_pretrain: bool = False):
        ...
        new_first_layer = nn.Conv2d(6, original_first_layer.out_channels,
                                    ...
                                    bias=False)
        # Copy RGB weights and initialize extra channels using Kaiming Normal
    ...
    def forward_features(self, x: torch.Tensor) -> Tuple[torch.Tensor, List[torch.Tensor]]:
        """
        Forward pass through the network, returning both final output and intermediate features.
        """
        x = self.stem(x)
        out = []
    ...
    def forward(self, image: torch.Tensor, mask: Optional[torch.Tensor] = None, *args, **kwargs) -> Tuple[torch.Tensor, List[torch.Tensor]]:
        """
        Forward function returns both feature map and multi-scale outputs.
        """
...
class DWConv(nn.Module):
    """Depthwise convolution for transformer blocks."""
...
class MixVisionTransformer(nn.Module):
    """SegFormer encoder with four stages."""
    def __init__(self, pretrain_path=None, img_size=512, patch_size=4, in_chans=3,
        ...
        # Extend first conv layer to support 6-channel input
...
class UpsampleConcatConv(nn.Module):
    """Feature fusion module with transposed convolutions."""
    def __init__(self):
        super(UpsampleConcatConv, self).__init__()

        # Upsampling paths for c2, c3, c4
...
class Mesorch(nn.Module):
    """
    Main model class combining frequency features, CNNs, and Transformers.
    Implements image manipulation localization.
    """
    ...
    def forward(self, image: torch.Tensor, mask: torch.Tensor = None, *args, **kwargs) -> Dict[str, torch.Tensor]:
        """
        Forward pass of the Mesorch model.

        Args:
            image: Input tensor of shape [B, 3, H, W]
            mask: Ground truth mask of shape [B, 1, H, W] (optional)
        
        Returns:
            output_dict: Dictionary containing loss, predicted masks, etc.
        """
        ...
        # Fuse features
        inputs = [a + b for a, b in zip(outs1, outs2)]
        ...
        loss = self.loss_fn(pred_mask, mask) if mask is not None else torch.tensor(0.)
        ...
        output_dict = {
            ...
            "visual_image": {"pred_mask": mask_pred}
        }
