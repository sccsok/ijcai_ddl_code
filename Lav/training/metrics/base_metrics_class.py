...
def calculate_metrics_for_train(label, output):
    ...
    # AUC and EER
    fpr, tpr, thresholds = metrics.roc_curve(label.squeeze().cpu().numpy(), prob.squeeze().cpu().numpy(),
                                             pos_label=1)
    ...
    return {'auc': auc, 'eer': eer, 'acc': accuracy, 'ap': ap}


def calculate_iou(pred_mask: torch.Tensor, true_mask: torch.Tensor, threshold: float = 0.5) -> torch.Tensor:
    """
    Compute IoU score between predicted and true binary masks.
    
    Args:
        pred_mask (torch.Tensor): Predicted mask tensor of shape [B, H, W] or [B, 1, H, W]
        true_mask (torch.Tensor): Ground truth binary mask tensor of shape [B, H, W] or [B, 1, H, W]
        threshold (float): Threshold to binarize the predicted mask

    Returns:
        torch.Tensor: IoU score per batch, shape [B]
    """
    ...
    union = (pred_mask + true_mask).sum(dim=1) - intersection
...
def calculate_f1_score(pred_mask: torch.Tensor, true_mask: torch.Tensor, threshold: float = 0.5) -> torch.Tensor:
    """
    Compute F1 score between predicted and true binary masks.
    ...
    Returns:
        torch.Tensor: F1 score per batch, shape [B]
    """
    # Ensure shape consistency
    if pred_mask.dim() == 4:
        pred_mask = pred_mask.squeeze(1)
    ...
    # Compute TP, FP, FN
    tp = (pred_mask * true_mask).sum(dim=1)
    fp = (pred_mask * (1 - true_mask)).sum(dim=1)
    fn = ((1 - pred_mask) * true_mask).sum(dim=1)
